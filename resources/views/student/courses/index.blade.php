...
@if($courses->count() > 0)
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full">
        @foreach($courses as $course)
            <div class="bg-white rounded-xl shadow-md hover:shadow-xl transition duration-300 p-4 h-64 flex flex-col justify-between">
                <div>
                ...
                <form action="{{ route('student.courses.enroll', ['id' => $course->COURSE_ID]) }}" method="POST" class="mt-auto">
                    @csrf
                    <button type="submit"
